...
@section('content')
    <div class="flex items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="font-extrabold text-2xl text-gray-900 leading-tight">Meal Templates</h2>
            <p class="text-sm text-gray-500 mt-1">Manage meal templates used for generating user plans.</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50">
                <tr class="text-left text-gray-600">
                    ...
                    <th class="p-4 font-semibold">Calories</th>
            ...
            <tbody class="divide-y">
                @forelse($items as $item)
                    <tr class="hover:bg-gray-50/60">
                        <td class="p-4 text-gray-700">{{ $item->meal_type }}</td>
                        <td class="p-4 font-semibold text-gray-900">{{ $item->name }}</td>
                        <td class="p-4 text-gray-700">{{ $item->calories }}</td>
                        ...
                        <td class="p-4">
                            <div class="flex justify-end gap-2 flex-wrap">
                                <a class="px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm font-semibold hover:bg-gray-50"
                                   href="{{ route('admin.meals.edit', $item) }}">
                                    Edit
                                </a>

                                <form method="POST"
                                      action="{{ route('admin.meals.destroy', $item) }}"
